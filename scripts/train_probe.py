...
# Feature generalizability classification
GENERALIZABLE_FEATURES = {
    "entropy_mean", "entropy_std", "margin_mean", "margin_min",
    "lp_mean", "seq_conf", "model_confidence"
}

NON_GENERALIZABLE_FEATURES = {
    "answer_len",      # Dataset-specific (varies by task)
    "is_unknown",      # Task-specific (SQuAD v2 only)
    "rescore_logp",    # Format-specific (requires JSON)
    "parsed_json_ok",  # Format-specific
    "parsed_p_true_ok" # Format-specific
}

def filter_scalar_keys(exclude_features=None):
    """Filter SCALAR_KEYS based on exclusion set"""
    if exclude_features:
        return [k for k in SCALAR_KEYS if k not in exclude_features]
    return SCALAR_KEYS

def build_df(rows, use_hidden=True, label_key="em", show_progress=True, exclude_features=None):
    ...
    # Filter scalar keys if requested
    scalar_keys = filter_scalar_keys(exclude_features)
    if exclude_features and show_progress:
        excluded = [k for k in SCALAR_KEYS if k not in scalar_keys]
        if excluded:
            console.print(f"  [yellow]Excluding features: {', '.join(excluded)}[/yellow]")
    ...
    for chunk_start in range(0, n_rows, chunk_size):
        ...
        for i in range(chunk_start, chunk_end):
            ...
            # Fill scalars (using filtered list)
...
def main():
    ...
    parser.add_argument("--exclude_features", type=str, default=None,
                        help="Comma-separated features to exclude (e.g., 'answer_len,is_unknown')")
    parser.add_argument("--only_generalizable", action="store_true",
                        help="Use only generalizable features (excludes answer_len, is_unknown, format-specific)")
    ...
    if args.exclude_features:
        ...
        console.print(f"[yellow]Excluding features: {', '.join(sorted(exclude_features))}[/yellow]")
    elif args.only_generalizable:
        exclude_features = NON_GENERALIZABLE_FEATURES
        console.print(f"[yellow]Using only generalizable features[/yellow]")
    ...
    for mk_idx, mk in enumerate(model_keys, 1):
        ...
        df_tr, y_tr, _ = build_df(rows_train, use_hidden=args.use_hidden, label_key=args.label_key, exclude_features=exclude_features)
        df_va, y_va, _ = (None, None, None)
        df_te, y_te, _ = (None, None, None)

        if rows_val:
            df_va, y_va, _ = build_df(rows_val, use_hidden=args.use_hidden, label_key=args.label_key, exclude_features=exclude_features)
        if rows_test:
            df_te, y_te, _ = build_df(rows_test, use_hidden=args.use_hidden, label_key=args.label_key, exclude_features=exclude_features)
