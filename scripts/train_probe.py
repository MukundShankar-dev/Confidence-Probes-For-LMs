...
# Now with --xform_dump_attn to export averaged attention maps for the Transformer probe.
...
# ---- Small Transformer probe (PyTorch) ----
# We treat feature *groups* as tokens:
# tokens = [h_last_256, h_pool_256, h_last_mid_256, h_pool_mid_256, scalars_256]
# where 'scalars_256' = scalars zero-padded (the model also projects 13→256).
# If --use_hidden is False: tokens = [scalars_256] only.
...
class EncoderBlock(nn.Module):
    def __init__(self, d_model=256, nhead=4, d_ff=512, p_drop=0.1):
        super().__init__()
...
class TinyTransformerProbe(nn.Module):
    def __init__(self, use_hidden=True, d_model=256, nhead=4, num_layers=2, d_ff=512, p_drop=0.1):
        super().__init__()
        self.use_hidden = use_hidden
        self.d_model = d_model
        self.scalar_proj = nn.Linear(13, d_model)   # 13 scalar features → d_model
        self.encoders = nn.ModuleList([EncoderBlock(d_model, nhead, d_ff, p_drop) for _ in range(num_layers)])
    ...
    def forward(self, tokens_256, return_attn=False):
        """
        tokens_256: [B, T, D] where last token contains 13 scalars padded to D
        We internally re-project the last token from raw 13 scalars -> D before encoding.
        If return_attn=True, returns (p, attn_per_layer) with attn_per_layer list of [B,H,T,T].
        """
        B, T, D = tokens_256.shape
        # recompose: last token is scalars padded; take its first 13 dims, project
        scal_raw = tokens_256[:, -1, :13]                      # [B,13]
        scal_proj = self.scalar_proj(scal_raw).unsqueeze(1)   # [B,1,D]
        if T == 1:
            x = scal_proj
        else:
            x = torch.cat([tokens_256[:, :-1, :], scal_proj], dim=1)  # [B,T,D]
...
def build_transformer_tokens(X_df: pd.DataFrame, use_hidden: bool):
    ...
    # add scalar token as 256-d (zero-padded)
    pad_to = 256
    scal_tok = scalars
...
def train_transformer_probe(X_train_df, y_train, X_val_df=None, y_val=None, use_hidden=True, epochs=20, batch_size=256, lr=1e-3, device=None, out_dir=None):
    ...
    model = TinyTransformerProbe(use_hidden=use_hidden).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=1e-4)
    loss_fn = nn.MSELoss()  # Brier-style
    ...
    best = {"brier": float("inf"), "state_dict": None}
    ...
    if out_dir is not None:
        save = {
            ...
            "d_model": 256, "nhead": 4, "num_layers": 2, "d_ff": 512,
...
def xform_predict_proba(model_obj, X_df, use_hidden=True):
...
def dump_attention_maps(model_obj, X_df, use_hidden, out_dir: Path, max_batches=8, batch_size=256):
    ...
    # token labels
    tok_labels = (VECTOR_KEYS + ["scalars_256"]) if use_hidden else ["scalars_256"]
    T = len(tok_labels)
    ...
    with torch.no_grad():
        ...
        for (xb,) in ld:
            ...
            # We need to run the same token replacement (scalar projection) as in forward:
            B, TT, D = xb.shape
            scal_raw = xb[:, -1, :13]
            scal_proj = model.scalar_proj(scal_raw).unsqueeze(1)
            x = scal_proj if (TT == 1) else torch.cat([xb[:, :-1, :], scal_proj], dim=1)

            attn_list = []
            x_cur = x
            for layer in model.encoders:
                x_cur, attn = layer(x_cur, need_attn=True)  # attn [B,H,T,T]
        ...
        for li in range(layer_count):
            ...
            # normalize rows to sum to 1 for readability
            ...
            plt.close()
            outs.append({"layer": li+1, "tokens": tok_labels, "path_csv": str(out_dir / f"attn_layer{li+1}.csv"),
                         "path_png": str(out_dir / f"attn_layer{li+1}.png")})

        with open(out_dir / "attn_summary.json", "w", encoding="utf-8") as f:
            json.dump({"tokens": tok_labels, "layers": outs}, f, indent=2)
...
def fit_and_eval_probe(probe_type: str, X_tr, y_tr, X_va, y_va, X_te, y_te, use_hidden, out_root: Path,
                       random_state=7, xform_epochs=20, xform_dump_attn=False, xform_attn_split="val",
                       xform_attn_batches=8):
    """
    Trains one probe type, writes artifacts under out_root / probe_<type>/
    Returns a dict of split metrics and artifact paths.
    """
    ...
    if probe_type == "mlp":
        ...
        model = train_transformer_probe(
            X_tr, y_tr, X_va, y_va,
            use_hidden=use_hidden, epochs=xform_epochs, out_dir=probe_dir
        )
        saver = lambda: torch.save({
            ...
            "vector_keys": VECTOR_KEYS if use_hidden else [],
            "packed_dim": 256,
            "arch": {"d_model":256,"nhead":4,"num_layers":2,"d_ff":512}
        }, probe_dir / "probe_model.pt")
        predict_proba = lambda X: xform_predict_proba(model, X, use_hidden=use_hidden)
        ...
        if xform_dump_attn:
            ...
            dump_attention_maps(model, X_for_attn, use_hidden, probe_dir, max_batches=xform_attn_batches)
    ...
    def do_eval(X, y, ids, name):
        ...
        metrics, prec, rec = evaluate_from_proba(proba, y, name)
        split_evals.append(metrics)
        # preds
        out = pd.DataFrame({"id": ids, "y_true": y, "p_right": proba})
        out.to_csv(probe_dir / f"pred_{name}.csv", index=False)
        # plots
        plot_curves(probe_dir, name, y, proba)

    do_eval(X_tr, y_tr, X_tr.index if isinstance(X_tr.index, np.ndarray) else np.arange(len(y_tr)), "train")
    if X_va is not None:
        do_eval(X_va, y_va, np.arange(len(y_va)), "val")
    if X_te is not None:
        do_eval(X_te, y_te, np.arange(len(y_te)), "test")

    # Save report + metadata for eval_probe.py
    ...
    meta = {
        ...
        "plots": {
            "train": {
                "roc": str(probe_dir / "roc_train.png"),
                "pr":  str(probe_dir / "pr_train.png"),
            },
            "val": {
                "roc": str(probe_dir / "roc_val.png") if (probe_dir / "roc_val.png").exists() else None,
                "pr":  str(probe_dir / "pr_val.png")  if (probe_dir / "pr_val.png").exists() else None,
            },
            "test": {
                "roc": str(probe_dir / "roc_test.png") if (probe_dir / "roc_test.png").exists() else None,
...
def main():
    ...
    parser.add_argument("--xform_epochs", type=int, default=20)
    # NEW: attention export toggles
    ...
    for mk in model_keys:
        ...
        # Save feature order once at backend level
